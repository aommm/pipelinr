    ...
    eval(transformations) {
        ...
        const val = _.reduce(transformations, function(val, transformation, i) {
            ...
            if (_.isArray(transformation)) {
                ...
                const indices = _.range(0, val.length);
                nextVal = _.zipWith(transformation, val, indices, function(transformation, val, i) {
                    const nextVal = _this.evalOne(transformation, val, i);
                    return nextVal;
                });
            }
            else {
                nextVal = _this.evalOne(transformation, val, 0);
            }
        ...
        if (val instanceof NoValueError) {
            return undefined;
        }
        return val;
    }
